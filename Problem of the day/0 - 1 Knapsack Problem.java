    ...
    static int knapSack(int W, int wt[], int val[]) {
        ...
        for (int i = 1; i <= n; i++) {
            for (int w = 1; w <= W; w++) {
                ...
                // w, we have two options: either we include the current item or we don't.
                if (wt[i - 1] <= w) {
                    dp[i][w] = Math.max(val[i - 1] + dp[i - 1][w - wt[i - 1]], dp[i - 1][w]);
                } else {
                    // If the weight of the current item is more than the current capacity w,
                    // we cannot include this item, so we just carry forward the value without it.
                    dp[i][w] = dp[i - 1][w];
                }
            }
        }

        // The answer will be in dp[n][W], where n is the total number of items and W is the capacity.
        return dp[n][W];
    }
}
